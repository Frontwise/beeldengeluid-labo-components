...
class SearchHit extends React.Component {
	...
	gotoItemDetails() {
		if (
			this.props.itemDetailsPath &&
			this.props.data.formattedData.resourceId
		) {
			ComponentUtil.pushItemToLocalStorage(
				"visitedHits",
				this.props.data.formattedData.resourceId
			);
	...
	render() {
		//FIXME make sure that this snippet already contains the highlight data!
		const snippet = this.props.data.collectionConfig.getResultSnippetData(
			this.props.data.formattedData,
			this.props.data.numHighlights
		);
		const collectionMediaTypes = this.props.data.collectionConfig.getCollectionMediaTypes();
		const selectedRows = ComponentUtil.getJSONFromLocalStorage(
			"selectedRows"
		);
		const visitedItems = ComponentUtil.getJSONFromLocalStorage(
			"visitedHits"
		);
		...
		if (
			visitedItems &&
			visitedItems.find(
				item => item === this.props.data.formattedData.resourceId
			)
		) {
			classNames.push("visitedItem");

			visitedIcon = (<div className={IDUtil.cssClassName("visited-icon", this.CLASS_PREFIX)} title="Visited" />);
		}
		...
		return (
			<div className={classNames.join(" ")}>
				...
				<SearchSnippet
					onClick={this.gotoItemDetails.bind(this)}
					data={snippet}
