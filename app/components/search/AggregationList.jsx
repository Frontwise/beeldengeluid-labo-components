...
class AggregationList extends React.Component {
    ...
    toggleShowMore = aggr => {
        ...
        states[aggr.field] =
            states[aggr.field] === undefined ? true : !states[aggr.field];
    ...
    toggleExcludeFacets = index => {
        this.props.desiredFacets[index]["exclude"] = !this.props.desiredFacets[
            index
        ]["exclude"];
    ...
    getShowExtraOptions = aggrField => {
        return this.state.showExtraOptions[aggrField] === undefined
            ? false
            : this.state.showExtraOptions[aggrField];
    };

    toggleExtraOptions = aggrField => {
        const showExtraOptions = this.state.showExtraOptions;
        showExtraOptions[aggrField] =
            showExtraOptions[aggrField] === undefined
                ? true
                : !showExtraOptions[aggrField];
        ...
        desiredFacets.forEach((da, index) => {
            ...
            if (!isEmptyAggr) {
                ...
                facets = this.props.aggregations[da.field].map(facet => {
                    ...
                    let hidden =
                        this.state.showAllModes[da.field] === undefined
                            ? true
                            : !this.state.showAllModes[da.field];
                    if (hidden && visibleFacets < this.minToShow) {
                        hidden = false;
                    }

                    if (
                        this.props.selectedFacets[da.field] &&
                        this.props.selectedFacets[da.field].indexOf(
                            facet.key
                        ) !== -1
                    ) {
                        isSelected = true;
            ...
            uiData.push({
                ...
                title:
                    da.title ||
                    this.props.collectionConfig.toPrettyFieldName(da.field), // show user generated title or else the automated prettified title
            ...
            return (
                <li
                    ...
                    className={classNames(
                        { selected: f.selected, exclude: aggr.exclude },
                        IDUtil.cssClassName("facet-item", this.CLASS_PREFIX)
                    )}
                    onClick={this.toggleSelectedFacet.bind(
                        this,
                        aggr.field,
                        f.key
                    )}
    ...
    renderSortButton = (aggr, sortType) => {
        ...
        return (
            <div
                ...
                title={title}
                onClick={this.setFacetSortMode.bind(
                    this,
                    aggr.field,
                    sortType,
                    newDirection
            ...
            >
                ...
                >
                    <input
                        ...
                        id={aggr.field}
                        checked={
                            this.props.desiredFacets[aggr.index]["exclude"]
                        }
                        onChange={this.toggleExcludeFacets.bind(
                            this,
                            aggr.index
                        )}
                    />
                ...
                <button
                    className={classNames(
                        "btn",
                        IDUtil.cssClassName("hide-button", this.CLASS_PREFIX)
                    )}
    ...
    renderToggleShowMore = aggr => {
        const showAll =
            this.state.showAllModes[aggr.field] === undefined
                ? false
                : this.state.showAllModes[aggr.field];
        ...
        return (
            <a
                ...
                onClick={this.toggleShowMore.bind(this, aggr)}
            >
    ...
    renderEmptyBlocks = (aggr, index) => {
        return (
            <div
                className={IDUtil.cssClassName(
                    "facet-block aggregation-no-results",
                    this.CLASS_PREFIX
                )}
                key={"facet__" + aggr.index}
                id={"index__" + aggr.index}
            ...
            >
                <div>
                    <p>
                        ...
                        <u>{this.currentFacet}</u>". You can bring it back by
